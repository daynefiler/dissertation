# A novel copy number variant algorithm {#mcCNV}
...
9. Adjust p-values using the Benjamini–Hochberg procedure [@benjamini:1995aa] and filter to a final call-set such that adjusted p-values fall below some threshold, $\alpha$.
...
(ref:wgsCallSbjScap) Number of CNV calls by subject and algorithm for the 'WGS' pool.
...
kable(wgsCallBySbj,
      ...
      caption.short = '(ref:wgsCallSbjScap)',
      label = "wgsCallSbj",
      format.args = list(big.mark = ",", scientific = FALSE),
      booktabs = TRUE) %>%
  kable_classic() %>%
...
(ref:predMetScap) mcCNV and ExomeDepth performance predicting calls from genome sequencing.
